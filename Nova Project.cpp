...
public:
    int idx;
    int box;
    point_box(int arg_idx) {
        idx = arg_idx;
        box = -1;
    }
    bool operator < (const point_box& rhs)
        const {
        return(box < rhs.box);
    }
};
...
void getMinMax(vector< vec3<double> >& inCloud, vec3<double>& minp, vec3<double>& maxp) {
    for (int i = 0; i < inCloud.size(); i++) {
        ...
        maxp.z = std::max(maxp.z, inCloud[i].y);
    }
}

void filterPoints(vec3<int>& leafCount, vector< vec3<double> >& inCloud, vector< vec3<double> >& outCloud, vector< point_box > indices) {
    vec3<double> minp(DBL_MAX, DBL_MAX, DBL_MAX), maxp(-DBL_MAX, -DBL_MAX, -DBL_MAX);
    getMinMax(inCloud, minp, maxp);

    vec3<double> leafSize((maxp.x - minp.x) / leafCount.x, (maxp.y - maxp.y) / leafCount.y, (maxp.z - minp.z) / leafCount.z);
    ...
    for (int i = 0; i < inCloud.size(); i++) {
        int abc0 = static_cast<int> (floor(inCloud[i].x * inv_leafSize.x) - minb.x);
        int abc1 = static_cast<int> (floor(inCloud[i].y * inv_leafSize.y) - minb.y);
        int abc2 = static_cast<int> (floor(inCloud[i].z * inv_leafSize.z) - minb.z);
        int idx = abc0 * divb_mul.x + abc1 * divb_mul.y + abc2 * divb_mul.z;
        indices[i].box = idx;
    }
    sort(indices.begin(), indices.end(), less<point_box>());
...
int main(int argc, char* argv[]) {
    ...
    vector<point_box> indices;
    ifstream infile(argv[1]);
    while (infile >> point.x >> point.y >> point.z) {
        indices.push_back(point_box(inCloud.size()));
